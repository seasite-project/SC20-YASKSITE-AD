...
namespace {
    class WaveBase_offsite : public yc_solution_with_radius_base {
        protected:
            ...
            yc_var_proxy vel  = yc_var_proxy("vel", get_soln(), {x, y, z }); // 3D var.
            yc_var_proxy prev_data  = yc_var_proxy("prev_data", get_soln(), {x, y, z }); // 3D var.
    ...
    class Wave3dStencil_offsite : public WaveBase_offsite {
        protected:
        ...
        public:
            Wave3dStencil_offsite(int radius=4) :
                WaveBase_offsite("Wave3D_offsite", radius) { }
            Wave3dStencil_offsite(const string& name, int radius=4) :
                WaveBase_offsite(name, radius) { }
    };

    YS_REGISTER_STENCIL_RADIUS("Wave3D_offsite", Wave3dStencil_offsite);
}
#endif
